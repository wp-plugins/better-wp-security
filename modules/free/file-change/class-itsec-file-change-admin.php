...
class ITSEC_File_Change_Admin {
	...
	public function add_admin_meta_boxes() {
		...
		$title = __( 'File Change Detection', 'ithemes-security' );
		...
		if ( isset( get_current_screen()->id ) && ( strpos( get_current_screen()->id, 'security_page_toplevel_page_itsec_settings' ) !== false || strpos( get_current_screen()->id, 'security_page_toplevel_page_itsec_logs' ) !== false ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'button_text'  => __( 'Scan Files Now', 'ithemes-security' ),
					'scanning_button_text'  => __( 'Scanning... (this could take a while on a large site)', 'ithemes-security' ),
					'no_changes'   => __( 'No changes were detected.', 'ithemes-security' ),
					'changes'      => __( 'Changes were detected. Please check the log page for details.', 'ithemes-security' ),
					'error'        => __( 'An error occured. Please try again later', 'ithemes-security' ),
					'ABSPATH'      => ABSPATH,
	...
	public function dashboard_status( $statuses ) {
		...
		if ( $this->settings['enabled'] === true ) {
			...
			$status       = array( 'text' => __( 'Your site will detect changes to your files.', 'ithemes-security' ), 'link' => '#itsec_file_change_enabled', );

		} else {

			$status_array = 'medium';
			$status       = array( 'text' => __( 'Your website is not looking for changed files. Consider turning on file change detections.', 'ithemes-security' ), 'link' => '#itsec_file_change_enabled', );
			...
			if ( ! function_exists( 'itsec_intrusion_warning' ) ) {
				...
				function itsec_intrusion_warning() {
					...
					printf(
						...
						__( 'has noticed a change to some files in your WordPress site. Please review the logs to make sure your system has not been compromised.', 'ithemes-security' ),
						__( 'View Logs', 'ithemes-security' ),
						__( 'Dismiss Warning', 'ithemes-security' )
	...
	public function email() {
		...
		$content .= '<label for="itsec_file_change_email"> ' . __( 'Email file change notifications', 'ithemes-security' ) . '</label>';
	...
	public function enabled() {
		...
		$content .= '<label for="itsec_file_change_enabled"> ' . __( 'Enable File Change detection', 'ithemes-security' ) . '</label>';
	...
	public function split() {
		...
		echo '<label for="itsec_file_change_split"> ' . __( 'Split file checking into chunks.', 'ithemes-security' ) . '</label>';
		echo '<p class="description"> ' . __( 'Splits file checking into 7 chunks (plugins, themes, wp-admin, wp-includes, uploads, the rest of wp-content and everything that is left over) and divides the checks evenly over the course of a day. This feature may result in more notifications but will allow for the scanning of bigger sites to continue even on a lower-end web host.', 'ithemes-security' ) . '</p>';
	...
	public function file_change_form( $origin ) {
		...
		if ( isset( $this->settings['enabled'] ) && $this->settings['enabled'] === true ) {
			...
			echo '<p>' . __( "Press the button below to scan your site's files for changes. Note that if changes are found this will take you to the logs page for details.", 'ithemes-security' ) . '</p>';
	...
	public function file_list() {
		...
		$content = '<p class="description">' . __( 'Exclude files or folders by clicking the red minus next to the file or folder name.', 'ithemes-security' ) . '</p>';
	...
	public function logs_metabox() {
		...
		if ( isset( $this->settings['enabled'] ) && $this->settings['enabled'] === true ) {
			...
			if ( date( 'j', $next_run_raw ) == date( 'j', $itsec_globals['current_time'] ) ) {
				$next_run_day = __( 'Today', 'ithemes-security' );
			} else {
				$next_run_day = __( 'Tomorrow', 'ithemes-security' );
			}
			...
			echo '<p><em>*' . __( 'Automatic file change scanning is triggered by a user visiting your page and may not happen exactly at the time listed.', 'ithemes-security' ) . '</em>';
	...
	public function method() {
		...
		echo '</select><br />';
		echo '<label for="itsec_file_change_method"> ' . __( 'Include/Exclude Files', 'ithemes-security' ) . '</label>';
	...
	public function notify_admin() {
		...
		$content .= '<label for="itsec_file_change_notify_admin"> ' . __( 'Display file change admin warning', 'ithemes-security' ) . '</label>';
		$content .= '<p class="description">' . __( 'Disabling this feature will prevent the file change warning from displaying to the site administrator in the WordPress Dashboard. Note that disabling both the error message and the email notification will result in no notifications of file changes. The only way you will be able to tell is by manually checking the log files.', 'ithemes-security' ) . '</p>';
	...
	public function types() {
		...
		$content .= '<label for="itsec_file_change_types"> ' . __( 'File types listed here will not be checked for changes. While it is possible to change files such as images it is quite rare and nearly all known WordPress attacks exploit php, js and other text files.', 'ithemes-security' ) . '</label>';
	...
	public function initialize_admin() {
		...
		add_settings_field(
			...
			__( 'Split File Scanning', 'ithemes-security' ),
			array( $this, 'split' ),
			'security_page_toplevel_page_itsec_settings',
			'file_change-settings'
		);
		...
		add_settings_field(
			...
			__( 'Display file change admin warning', 'ithemes-security' ),
	...
	public function register_logger_metaboxes( $metaboxes ) {
		...
		$metaboxes[] = array(
			'module'   => 'file_change',
			'title'    => __( 'File Change History', 'ithemes-security' ),
