...
final class ITSEC_File_Change_Log extends ITSEC_WP_List_Table {
	...
	function column_memory( $item ) {
		...
		return $item['memory'] . __( 'MB', 'ithemes-security' );
	...
	function column_detail( $item ) {
		...
		if ( $item['added'] > 0 || $item['removed'] > 0 || $item['changed'] > 0 ) {
			...
			echo '<a href="itsec-log-file-change-row-' . $item['detail'] . '" class="dialog">' . __( 'Details', 'ithemes-security' ) . '</a>';

			$content = '<div id="itsec-log-file-change-row-' . $item['detail'] . '" style="display:none;">';

			$content .= '<h3>' . __( 'Files Added', 'ithemes-security' ) . '</h3>';

			$content .= '<ol class="file_change_detail_list">';

			foreach ( $item['added_detail'] as $file => $details ) {
				$content .= '<li class="file_change_detail"><strong>' . __( 'File', 'ithemes-security' ) . '</strong>: ' . $file . '<br /><strong>' . __( 'Date', 'ithemes-security' ) . '</strong>: ' . date( ' Y-m-d g:i a', $details['mod_date'] ) . '</li>';
			}

			$content .= '</ol>';

			$content .= '<h3>' . __( 'Files Removed', 'ithemes-security' ) . '</h3>';
			...
			$content .= '<h3>' . __( 'Files Changed', 'ithemes-security' ) . '</h3>';
