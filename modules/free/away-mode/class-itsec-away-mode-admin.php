...
class ITSEC_Away_Mode_Admin {
	...
	public function add_admin_meta_boxes() {
		...
		$title = __( 'Away Mode', 'ithemes-security' );
	...
	public function away_mode_enabled() {
		...
		$content .= '<label for="itsec_away_mode_enabled"> ' . __( 'Enable away mode', 'ithemes-security' ) . '</label>';
	...
	public function away_mode_end_date() {
		...
		$content .= '<label class="end_date_field" for="itsec_away_mode_end_date"> ' . __( 'Set the date at which the admin dashboard should become available', 'ithemes-security' ) . '</label>';
	...
	public function away_mode_end_time() {
		...
		$content .= '</select><br>';
		$content .= '<label for="itsec_away_mode_away_mod_end_time"> ' . __( 'Set the time at which the admin dashboard should become available again.', 'ithemes-security' ) . '</label>';
	...
	public function away_mode_start_date() {
		...
		$content .= '<label class="start_date_field" for="itsec_away_mode_start_date"> ' . __( 'Set the date at which the admin dashboard should become unavailable', 'ithemes-security' ) . '</label>';
	...
	public function away_mode_start_time() {
		...
		$content .= '</select><br>';
		$content .= '<label for="itsec_away_mode_away_mod_start_time"> ' . __( 'Set the time at which the admin dashboard should become unavailable.', 'ithemes-security' ) . '</label>';
	...
	public function away_mode_type() {
		...
		$content .= '<option value="1" ' . selected( $this->settings['type'], 1, false ) . '>' . __( 'Daily', 'ithemes-security' ) . '</option>';
		$content .= '<option value="2" ' . selected( $this->settings['type'], 2, false ) . '>' . __( 'One Time', 'ithemes-security' ) . '</option>';
		$content .= '</select><br>';
		$content .= '<label for="itsec_away_mode_type"> ' . __( 'Select the type of restriction you would like to enable', 'ithemes-security' ) . '</label>';
	...
	public function dashboard_status( $statuses ) {
		...
		if ( $this->settings['enabled'] === true ) {
			...
			$status       = array( 'text' => __( 'Away Mode is enabled and your WordPress Dashboard is not available when you will not be needing it.', 'ithemes-security' ), 'link' => '#itsec_away_mode_enabled', );

		} else {

			$status_array = 'medium';
			$status       = array( 'text' => __( 'Your WordPress Dashboard is available 24/7. Do you really update 24 hours a day? Consider using Away Mode.', 'ithemes-security' ), 'link' => '#itsec_away_mode_enabled', );
	...
	public function initialize_admin() {
		...
		add_settings_field(
			'itsec_away_mode[start_date]', __( 'Start Date', 'ithemes-security' ),
		...
		add_settings_field(
			'itsec_away_mode[start_time]', __( 'Start Time', 'ithemes-security' ),
