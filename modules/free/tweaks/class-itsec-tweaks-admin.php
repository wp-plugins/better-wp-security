...
class ITSEC_Tweaks_Admin {
	...
	public function add_admin_meta_boxes() {
		...
		$id    = 'tweaks_system';
		$title = __( 'System Tweaks', 'ithemes-security' );
		...
		$id    = 'tweaks_wordpress';
		$title = __( 'WordPress Tweaks', 'ithemes-security' );
		...
		if ( is_multisite() ) {
			...
			$id    = 'tweaks_multisite';
			$title = __( 'Multi-site Tweaks', 'ithemes-security' );
	...
	public function tweaks_server_directory_browsing() {
		...
		$content .= '<label for="itsec_tweaks_server_directory_browsing">' . __( 'Disable Directory Browsing', 'ithemes-security' ) . '</label>';
	...
	public function tweaks_server_long_url_strings() {
		...
		$content .= '<label for="itsec_tweaks_server_long_url_strings">' . __( 'Filter Long URL Strings', 'ithemes-security' ) . '</label>';
	...
	public function tweaks_server_non_english_characters() {
		...
		$content .= '<label for="itsec_tweaks_server_non_english_characters">' . __( 'Filter Non-English Characters', 'ithemes-security' ) . '</label>';
	...
	public function tweaks_server_protect_files() {
		...
		$content .= '<label for="itsec_tweaks_server_protect_files">' . __( 'Protect System Files', 'ithemes-security' ) . '</label>';
	...
	public function tweaks_server_request_methods() {
		...
		$content .= '<label for="itsec_tweaks_server_request_methods">' . __( 'Filter Request Methods', 'ithemes-security' ) . '</label>';
	...
	public function tweaks_server_suspicious_query_strings() {
		...
		$content .= '<label for="itsec_tweaks_server_suspicious_query_strings">' . __( 'Filter Suspicious Query Strings in the URL', 'ithemes-security' ) . '</label>';
	...
	public function tweaks_server_write_permissions() {
		...
		$content .= '<label for="itsec_tweaks_server_write_permissions">' . __( 'Remove File Writing Permissions', 'ithemes-security' ) . '</label>';
	...
	public function tweaks_wordpress_disable_unused_author_pages() {
		...
		$content .= '<label for="itsec_tweaks_server_disable_unused_author_pages"> ' . __( "Disables a user's author page if their post count is 0.", 'ithemes-security' ) . '</label>';
		$content .= '<p class="description"> ' . __( "This makes it harder for bots to determine usernames by disabling post archives for users that don't post to your site.", 'ithemes-security' ) . '</p>';
	...
	public function tweaks_wordpress_force_unique_nicename() {
		...
		$content .= '<label for="itsec_tweaks_server_force_unique_nicename"> ' . __( 'Force users to choose a unique nickname', 'ithemes-security' ) . '</label>';
		$content .= '<p class="description"> ' . __( "This forces users to choose a unique nickname when updating their profile or creating a new account which prevents bots and attackers from easily harvesting user's login usernames from the code on author pages. Note this does not automatically update existing users as it will affect author feed urls if used.", 'ithemes-security' ) . '</p>';
	...
	public function tweaks_wordpress_login_errors() {
		...
		$content .= '<label for="itsec_tweaks_server_login_errors"> ' . __( 'Disable login error messages', 'ithemes-security' ) . '</label>';
	...
	public function tweaks_wordpress_comment_spam() {
		...
		$content .= '<label for="itsec_tweaks_server_comment_spam">' . __( 'Reduce Comment Spam', 'ithemes-security' ) . '</label>';
	...
	public function tweaks_wordpress_core_updates() {
		...
		$content .= '<label for="itsec_tweaks_server_core_updates">' . __( 'Hide Core Update Notifications', 'ithemes-security' ) . '</label>';
	...
	public function tweaks_wordpress_disable_xmlrpc() {
		...
		$content .= '<label for="itsec_tweaks_server_disable_xmlrpc">' . __( 'Disable XML-RPC', 'ithemes-security' ) . '</label>';
	...
	public function tweaks_wordpress_edituri_header() {
		...
		$content .= '<label for="itsec_tweaks_server_edituri_header">' . __( 'Remove the RSD (Really Simple Discovery) header. ', 'ithemes-security' ) . '</label>';
	...
	public function tweaks_wordpress_file_editor() {
		...
		$content .= '<label for="itsec_tweaks_server_file_editor">' . __( 'Disable File Editor', 'ithemes-security' ) . '</label>';
	...
	public function tweaks_wordpress_generator_tag() {
		...
		$content .= '<label for="itsec_tweaks_server_generator_tag">' . __( 'Remove WordPress Generator Meta Tag', 'ithemes-security' ) . '</label>';
	...
	public function tweaks_wordpress_plugin_updates() {
		...
		$content .= '<label for="itsec_tweaks_server_plugin_updates">' . __( 'Hide Plugin Update Notifications', 'ithemes-security' ) . '</label>';
	...
	public function tweaks_wordpress_random_version() {
		...
		$content .= '<label for="itsec_tweaks_server_random_version">' . __( 'Display Random Version', 'ithemes-security' ) . '</label>';
		$content .= '<p class="description">' . __( 'Where a WordPress version must be displayed, it will display a random WordPress version and will remove the WordPress version completely where possible.', 'ithemes-security' ) . '</p>';
	...
	public function tweaks_wordpress_safe_jquery() {
		...
		$content .= '<label for="itsec_tweaks_wordpress_safe_jquery">' . __( 'Enqueue a safe version of jQuery', 'ithemes-security' ) . '</label>';
		...
		$content .= sprintf(
			...
			__( 'Note that this only checks the homepage of your site and only for users who are logged in. This is done intentionally to save resources. If you think this is in error ', 'ithemes-security' ),
			site_url(),
			__( 'click here to check again.', 'ithemes-security' ),
			__( 'This will open your homepage in a new window allowing the plugin to determine the version of jQuery actually being used. You can then come back here and reload this page to see your version.', 'ithemes-security' )
		);
	...
	public function tweaks_wordpress_theme_updates() {
		...
		$content .= '<label for="itsec_tweaks_server_theme_updates">' . __( 'Hide Theme Update Notifications', 'ithemes-security' ) . '</label>';
	...
	public function tweaks_wordpress_uploads_php() {
		...
		$content .= '<label for="itsec_tweaks_server_uploads_php">' . __( 'Disable PHP in Uploads', 'ithemes-security' ) . '</label>';
	...
	public function tweaks_wordpress_wlwmanifest_header() {
		...
		$content .= '<label for="itsec_tweaks_server_wlwmanifest_header">' . __( 'Remove the Windows Live Writer header. ', 'ithemes-security' ) . '</label>';
	...
	public function dashboard_status( $statuses ) {
		...
		if ( isset( $this->settings['protect_files'] ) && $this->settings['protect_files'] === true ) {
			...
			$status       = array( 'text' => __( 'You are protecting common WordPress files from access.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_protect_files', );

		} else {

			$status_array = 'medium';
			$status       = array( 'text' => __( 'You are not protecting common WordPress files from access. Click here to protect WordPress files.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_protect_files', );
		...
		if ( ITSEC_Lib::get_server() != 'nginx' ) {
			...
			if ( isset( $this->settings['directory_browsing'] ) && $this->settings['directory_browsing'] === true && ITSEC_Lib::get_server() != 'nginx' ) {
				...
				$status       = array( 'text' => __( 'You have successfully disabled directory browsing on your site.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_directory_browsing', );

			} else {

				$status_array = 'low';
				$status       = array( 'text' => __( 'You have not disabled directory browsing on your site. Click here to prevent a user from seeing every file present in your WordPress site.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_directory_browsing', );
		...
		if ( isset( $this->settings['request_methods'] ) && $this->settings['request_methods'] === true ) {
			...
			$status_array = 'safe-low';
			$status       = array( 'text' => __( 'You are blocking HTTP request methods you do not need.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_request_methods', );

		} else {

			$status_array = 'low';
			$status       = array( 'text' => __( 'You are not blocking HTTP request methods you do not need. Click here to block extra HTTP request methods that WordPress should not normally need.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_request_methods', );
		...
		if ( isset( $this->settings['suspicious_query_strings'] ) && $this->settings['suspicious_query_strings'] === true ) {
			...
			$status_array = 'safe-medium';
			$status       = array( 'text' => __( 'Your WordPress site is blocking suspicious looking information in the URL.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_suspicious_query_strings', );

		} else {

			$status_array = 'medium';
			$status       = array( 'text' => __( 'Your WordPress site is not blocking suspicious looking information in the URL. Click here to block users from trying to execute code that they should not be able to execute.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_suspicious_query_strings', );
		...
		if ( isset( $this->settings['non_english_characters'] ) && $this->settings['non_english_characters'] === true ) {
			...
			$status_array = 'safe-low';
			$status       = array( 'text' => __( 'Your WordPress site is blocking non-english characters in the URL.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_non_english_characters', );

		} else {

			$status_array = 'low';
			$status       = array( 'text' => __( 'Your WordPress site is not blocking non-english characters in the URL. Click here to fix this.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_non_english_characters', );
		...
		if ( isset( $this->settings['long_url_strings'] ) && $this->settings['long_url_strings'] === true ) {
			...
			$status_array = 'safe-low';
			$status       = array( 'text' => __( 'Your installation does not accept long URLs.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_long_url_strings', );

		} else {

			$status_array = 'low';
			$status       = array( 'text' => __( 'Your installation accepts long (over 255 character) URLS. This can lead to vulnerabilities. Click here to fix this.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_long_url_strings', );
		...
		if ( isset( $this->settings['write_permissions'] ) && $this->settings['write_permissions'] === true ) {
			...
			$status_array = 'safe-low';
			$status       = array( 'text' => __( 'Your wp-config.php and .htaccess files are not writeable.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_write_permissions', );

		} else {

			$status_array = 'low';
			$status       = array( 'text' => __( 'Your wp-config.php and .htaccess files are writeable. This can lead to vulnerabilities. Click here to fix this.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_write_permissions', );
		...
		if ( isset( $this->settings['generator_tag'] ) && $this->settings['generator_tag'] === true ) {
			...
			$status_array = 'safe-medium';
			$status       = array( 'text' => __( 'Your WordPress installation is not telling every bot that you use WordPress.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_generator_tag', );

		} else {

			$status_array = 'medium';
			$status       = array( 'text' => __( 'Your WordPress installation is telling every bot that you use WordPress with a special "generator" tag. Click here to fix this.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_generator_tag', );
		...
		if ( isset( $this->settings['wlwmanifest_header'] ) && $this->settings['wlwmanifest_header'] === true ) {
			...
			$status_array = 'safe-low';
			$status       = array( 'text' => __( 'Your WordPress installation is not publishing the Windows Live Writer header.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_wlwmanifest_header', );

		} else {

			$status_array = 'low';
			$status       = array( 'text' => __( 'Your WordPress installation is publishing the Windows Live Writer header. Click here to fix this.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_wlwmanifest_header', );
		...
		if ( isset( $this->settings['edituri_header'] ) && $this->settings['edituri_header'] === true ) {
			...
			$status_array = 'safe-low';
			$status       = array( 'text' => __( 'Your WordPress installation is not publishing the Really Simple Discovery (RSD) header.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_edituri_header', );

		} else {

			$status_array = 'low';
			$status       = array( 'text' => __( 'Your WordPress installation is publishing the Really Simple Discovery (RSD) header. Click here to fix this.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_edituri_header', );
		...
		if ( is_multisite() ) {
			...
			if ( isset( $this->settings['theme_updates'] ) && $this->settings['theme_updates'] === true ) {
				...
				$status_array = 'safe-medium';
				$status       = array( 'text' => __( 'Your WordPress installation is not telling users who cannot update themes about theme updates.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_theme_updates', );

			} else {

				$status_array = 'medium';
				$status       = array( 'text' => __( 'Your WordPress installation is telling users who cannot update themes about theme updates. Click here to fix this.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_theme_updates', );
			...
			if ( isset( $this->settings['plugin_updates'] ) && $this->settings['plugin_updates'] === true ) {
				...
				$status_array = 'safe-medium';
				$status       = array( 'text' => __( 'Your WordPress installation is not telling users who cannot update plugins about plugin updates.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_plugin_updates', );

			} else {

				$status_array = 'medium';
				$status       = array( 'text' => __( 'Your WordPress installation is telling users who cannot update plugins about plugin updates. Click here to fix this.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_plugin_updates', );
			...
			if ( isset( $this->settings['core_updates'] ) && $this->settings['core_updates'] === true ) {
				...
				$status_array = 'safe-medium';
				$status       = array( 'text' => __( 'Your WordPress installation is not telling users who cannot update WordPress core about WordPress core updates.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_core_updates', );

			} else {

				$status_array = 'medium';
				$status       = array( 'text' => __( 'Your WordPress installation is telling users who cannot update WordPress core about WordPress core updates. Click here to fix this.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_core_updates', );
		...
		if ( isset( $this->settings['comment_spam'] ) && $this->settings['comment_spam'] === true ) {
			...
			$status_array = 'safe-medium';
			$status       = array( 'text' => __( 'Your WordPress installation is not allowing users without a user agent to post comments.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_comment_spam', );

		} else {

			$status_array = 'medium';
			$status       = array( 'text' => __( 'Your WordPress installation is allowing users without a user agent to post comments. Fix this to reduce comment spam.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_comment_spam', );
		...
		if ( isset( $this->settings['random_version'] ) && $this->settings['random_version'] === true ) {
			...
			$status_array = 'safe-low';
			$status       = array( 'text' => __( 'Version information is obscured to all non admin users.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_random_version', );

		} else {

			$status_array = 'low';
			$status       = array( 'text' => __( 'Users may still be able to get version information from various plugins and themes. Click here to fix this.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_random_version', );
		...
		if ( isset( $this->settings['file_editor'] ) && $this->settings['file_editor'] === true ) {
			...
			$status_array = 'safe-low';
			$status       = array( 'text' => __( 'Users cannot edit plugin and themes files directly from within the WordPress Dashboard.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_file_editor', );

		} else {

			$status_array = 'low';
			$status       = array( 'text' => __( 'Users can edit plugin and themes files directly from within the WordPress Dashboard. Click here to fix this.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_file_editor', );
		...
		if ( isset( $this->settings['disable_xmlrpc'] ) && $this->settings['disable_xmlrpc'] === true ) {
			...
			$status_array = 'safe-low';
			$status       = array( 'text' => __( 'XML-RPC is not available on your WordPress installation.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_disable_xmlrpc', );

		} else {

			$status_array = 'low';
			$status       = array( 'text' => __( 'XML-RPC is available on your WordPress installation. Attackers can use this feature to attack your site. Click here to disable access to XML-RPC.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_disable_xmlrpc', );
		...
		if ( isset( $this->settings['uploads_php'] ) && $this->settings['uploads_php'] === true ) {
			...
			$status_array = 'safe-medium';
			$status       = array( 'text' => __( 'Users cannot execute PHP from the uploads folder.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_uploads_php', );

		} else {

			$status_array = 'medium';
			$status       = array( 'text' => __( 'Users can execute PHP from the uploads folder.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_uploads_php', );
		...
		if ( $safe_jquery === true ) {
			...
			$status       = array( 'text' => __( 'The front page of your site is using a safe version of jQuery.', 'ithemes-security' ), 'link' => '#itsec_tweaks_wordpress_safe_jquery', );

		} elseif ( $safe_jquery === false ) {

			$status_array = 'high';
			$status       = array( 'text' => __( 'The front page of your site is not using a safe version of jQuery or the version of jQuery cannot be determined.', 'ithemes-security' ), 'link' => '#itsec_tweaks_wordpress_safe_jquery', );

		} else {

			$status_array = 'low';
			$status       = array( 'text' => __( 'Your version of jQuery has not been determined. Load your homepage while logged in to determine the version of jQuery you are using', 'ithemes-security' ), 'link' => '#itsec_tweaks_wordpress_safe_jquery', );
		...
		if ( isset( $this->settings['force_unique_nicename'] ) && $this->settings['force_unique_nicename'] === true ) {
			...
			$status_array = 'safe-high';
			$status       = array( 'text' => __( "User's nicknames are different from their display name.", 'ithemes-security' ), 'link' => '#itsec_tweaks_server_force_unique_nicename', );

		} else {

			$status_array = 'high';
			$status       = array( 'text' => __( "User nicknames may be the same as their login name. This means their login user name may be publicly available throughout the site.", 'ithemes-security' ), 'link' => '#itsec_tweaks_server_force_unique_nicename', );
		...
		if ( isset( $this->settings['login_errors'] ) && $this->settings['login_errors'] === true ) {
			...
			$status_array = 'safe-low';
			$status       = array( 'text' => __( 'Your login page is not giving out unnecessary information upon failed login.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_login_errors', );

		} else {

			$status_array = 'low';
			$status       = array( 'text' => __( 'Your login page is giving out unnecessary information upon failed login.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_login_errors', );
		...
		if ( isset( $this->settings['disable_unused_author_pages'] ) && $this->settings['disable_unused_author_pages'] === true ) {
			...
			$status_array = 'safe-medium';
			$status       = array( 'text' => __( 'User profiles for users without content are not publicly available.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_disable_unused_author_pages', );

		} else {

			$status_array = 'medium';
			$status       = array( 'text' => __( 'User profiles for users without content are publicly available. This can make it relatively easy to gain the username of important users.', 'ithemes-security' ), 'link' => '#itsec_tweaks_server_disable_unused_author_pages', );
	...
	public function initialize_admin() {
		...
		add_settings_field(
			...
			__( 'Force Unique Nickname', 'ithemes-security' ),
		...
		add_settings_field(
			...
			__( 'Disable Extra User Archives', 'ithemes-security' ),
	...
	public function metabox_tweaks_system() {
		...
		echo '<p>' . __( 'These are advanced settings that may be utilized to further strengthen the security of your WordPress site.', 'ithemes-security' ) . '</p>';
		echo '<p>' . __( 'Note: These settings are listed as advanced because they block common forms of attacks but they can also block legitimate plugins and themes that rely on the same techniques. When activating the settings below, we recommend enabling them one by one to test that everything on your site is still working as expected.', 'ithemes-security' ) . '</p>';
		echo '<p>' . __( 'Remember, some of these settings might conflict with other plugins or themes, so test your site after enabling each setting.', 'ithemes-security' ) . '</p>';
