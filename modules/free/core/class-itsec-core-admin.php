...
class ITSEC_Core_Admin {
	...
	public function add_plugin_meta_links( $meta ) {
		...
		$meta[] = '<a href="http://ithemes.com/security/ithemes-security-professional-setup" target="_blank">' . __( 'Get Pro Setup', 'ithemes-security' ) . '</a>';
		$meta[] = '<a href="http://ithemes.com/security" target="_blank">' . __( 'Get Support', 'ithemes-security' ) . '</a>';
	...
	public function metabox_get_backupbuddy() {
		...
		echo '<a href="http://ithemes.com/better-backups" class="button-secondary" target="_blank">' . __( 'Get BackupBuddy', 'ithemes-security' ) . '</a>';
	...
	public function metabox_get_started() {
		...
		echo '<h2>' . __( 'Watch the Walk-Through Video', 'ithemes-security' ) . '</h2>';
		echo '<a class="itsec-video-link" href="#" data-video-id="itsec_video"><img src="' . plugins_url( 'img/video.png', __FILE__ ) . '" /></a>';
		echo sprintf( '<p class="itsec-video-description">%s <a href="http://ithem.es/6y" target="_blank">%s</a> %s </p>', __( 'In this short video, we walk through', 'ithemes-security' ), __( 'how to get started securing your site', 'ithemes-security' ), __( 'with iThemes Security.', 'ithemes-security' ) );
		...
		echo '<h2>' . __( 'Website Security is a complicated subject, but we have experts that can help.', 'ithemes-security' ) . '</h2>';
		echo '<p>' . __( 'Get added peace of mind with professional support from our expert team and pro features to take your site security to the next level with iThemes Security Pro.', 'ithemes-security' ) . '</p>';
		echo '<p><a class="button-primary" href="http://www.ithemes.com/security" target="_blank">' . __( 'Get Support and Pro Features', 'ithemes-security' ) . '</a></p>';
		echo '</div>';
		echo '</div>';
			...
			if ( ! function_exists( 'ithemes_plugin_setup_notice' ) ) {
				...
				function ithemes_plugin_setup_notice() {
					...
					echo '<div class="updated" id="itsec_setup_notice"><span class="it-icon-itsec"></span>'
					     . $itsec_globals['plugin_name'] . ' ' . __( 'is almost ready.', 'ithemes-security' ) . '<a href="#" class="itsec-notice-button" onclick="document.location.href=\'?itsec_setup=yes&_wpnonce=' . wp_create_nonce( 'itsec-nag' ) . '\';">' . __( 'Secure Your Site Now', 'ithemes-security' ) . '</a><a target="_blank" href="http://ithemes.com/ithemes-security-4-is-here" class="itsec-notice-button">' . __( "See what's new in 4.0", 'ithemes-security' ) . '</a><a href="#" class="itsec-notice-hide" onclick="document.location.href=\'?itsec_setup=no&_wpnonce=' . wp_create_nonce( 'itsec-nag' ) . '\';">&times;</a>
						</div>';
