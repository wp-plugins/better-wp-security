...
final class ITSEC_Four_Oh_Four_Log extends ITSEC_WP_List_Table {
	...
	function column_uri( $item ) {
		...
		echo '<h3>' . __( 'Details for ' . $item['uri'], 'ithemes-security' ) . '</h3>';

		echo '<ol class="file_change_detail_list">';

		foreach ( $items as $item => $details ) {
			$data = maybe_unserialize( $details['log_data'] );
			echo '<li class="404_detail"><strong>' . __( 'Time', 'ithemes-security' ) . '</strong>: ' . $details['log_date'] . '<br /><strong>' . __( 'Host', 'ithemes-security' ) . '</strong>: ' . $details['log_host'] . '<br /><strong>' . __( 'Referrer', 'ithemes-security' ) . '</strong>: ' . $details['log_referrer'] . '<br /><strong>' . __( 'Query', 'ithemes-security' ) . '</strong>: ' . $data['query_string'] . '</li>';
		}
