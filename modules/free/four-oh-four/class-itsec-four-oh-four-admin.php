...
class ITSEC_Four_Oh_Four_Admin {
	...
	public function add_admin_meta_boxes() {
		...
		$title = __( '404 Detection', 'ithemes-security' );
	...
	public function dashboard_status( $statuses ) {
		...
		if ( $this->settings['enabled'] === true ) {
			...
			$status       = array( 'text' => __( 'Your site is protecting against bots looking for known vulnerabilities.', 'ithemes-security' ), 'link' => '#itsec_four_oh_four_enabled', );

		} else {

			$status_array = 'medium';
			$status       = array( 'text' => __( 'Your website is not protected against bots looking for known vulnerabilities. Consider turning on 404 protection.', 'ithemes-security' ), 'link' => '#itsec_four_oh_four_enabled', );
	...
	public function check_period() {
		...
		$content .= '<label for="itsec_four_oh_four_check_period"> ' . __( 'Minutes', 'ithemes-security' ) . '</label>';
	...
	public function enabled() {
		...
		if ( ( get_option( 'permalink_structure' ) == '' || get_option( 'permalink_structure' ) == false ) && ! is_multisite() ) {
			...
			$content .= '<label for="itsec_four_oh_four_enabled"> ' . __( 'Enable 404 detection', 'ithemes-security' ) . '</label>';
	...
	public function error_threshold() {
		...
		$content .= '<label for="itsec_four_oh_four_error_threshold"> ' . __( 'Errors', 'ithemes-security' ) . '</label>';
	...
	public function register_logger_metaboxes( $metaboxes ) {
		...
		if ( isset( $this->settings['enabled'] ) && $this->settings['enabled'] === true ) {
			...
			$metaboxes[] = array(
				'module'   => 'four_oh_four',
				'title'    => __( '404 Errors Found', 'ithemes-security' ),
