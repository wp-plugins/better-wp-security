...
class ITSEC_Database_Prefix_Admin {
	...
	public function dashboard_status( $statuses ) {
		...
		if ( $this->settings !== true ) {
			...
			$status       = array( 'text' => sprintf( '%s wp_.', __( 'Your database table prefix is not using', 'ithemes-security' ) ), 'link' => '#itsec_change_table_prefix', 'advanced' => true, );

		} else {

			$status_array = 'medium';
			$status       = array( 'text' => sprintf( '%s wp_.', __( 'Your database table prefix should not be', 'ithemes-security' ) ), 'link' => '#itsec_change_table_prefix', 'advanced' => true, );
	...
	public function metabox_advanced_settings() {
		...
		$content = '<p>' . __( 'By default, WordPress assigns the prefix "wp" to all tables in the database where your content, users, and objects exist. For potential attackers, this means it is easier to write scripts that can target WordPress databases as all the important table names for 95% of sites are already known. Changing the "wp" prefix makes it more difficult for tools that are trying to take advantage of vulnerabilities in other places to affect the database of your site.', 'ithemes-security' ) . '<strong>' . __( 'Before using this tool, we strongly recommend running a backup of your database.', 'ithemes-security' ) . '</strong></p>';
		...
		if ( isset( $itsec_globals['settings']['write_files'] ) && $itsec_globals['settings']['write_files'] === true ) {
			...
			if ( $this->settings === true ) { //Show the correct info
				...
				?>
				<p><strong><?php _e( 'Your database is using the default table prefix', 'ithemes-security' ); ?>
						<em>wp_</em>. <?php _e( 'You should change this.', 'ithemes-security' ); ?></strong></p>
			<?php
				...
				?>
				<p><?php _e( 'Your current database table prefix is', 'ithemes-security' ); ?>
				...
				<table class="form-table">
					...
					<tr valign="top">
						<th scope="row" class="settinglabel">
							<label
								for="itsec_change_table_prefix"><?php _e( 'Change Table Prefix', 'ithemes-security' ); ?></label>
						</th>
						<td class="settingfield">
							...
							<p class="description"><?php _e( 'Check this box to generate a new database table prefix.', 'ithemes-security' ); ?></p>
						</td>
					</tr>
					</tbody>
				</table>
