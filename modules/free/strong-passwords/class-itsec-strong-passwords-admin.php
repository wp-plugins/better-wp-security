...
class ITSEC_Strong_Passwords_Admin {
	...
	public function add_admin_meta_boxes() {
		...
		$title = __( 'Strong Passwords', 'ithemes-security' );
	...
	public function dashboard_status( $statuses ) {
		...
		if ( $this->settings['enabled'] === true && $this->settings['roll'] == 'subscriber' ) {
			...
			$status       = array( 'text' => __( 'You are enforcing strong passwords for all users.', 'ithemes-security' ), 'link' => '#itsec_strong_passwords_enabled', );

		} elseif ( $this->settings['enabled'] === true ) {

			$status_array = 'low';
			$status       = array( 'text' => __( 'You are enforcing strong passwords, but not for all users.', 'ithemes-security' ), 'link' => '#itsec_strong_passwords_enabled', );

		} else {

			$status_array = 'high';
			$status       = array( 'text' => __( 'You are not enforcing strong passwords for any users.', 'ithemes-security' ), 'link' => '#itsec_strong_passwords_enabled', );
	...
	public function metabox_strong_passwords_settings() {
		...
		echo '<p>' . __( 'Force users to use strong passwords as rated by the WordPress password meter.', 'ithemes-security' ) . '</p>';
	...
	public function strong_passwords_enabled() {
		...
		$content .= '<label for="itsec_strong_passwords_enabled"> ' . __( 'Enable strong password enforcement.', 'ithemes-security' ) . '</label>';
