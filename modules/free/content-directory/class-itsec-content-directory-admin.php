...
class ITSEC_Content_Directory_Admin {
	...
	public function dashboard_status( $statuses ) {
		...
		if ( $this->settings === true ) {
			...
			$status       = array( 'text' => __( 'You have renamed the wp-content directory of your site.', 'ithemes-security' ), 'link' => '#itsec_enable_content_dir', 'advanced' => true, );

		} else {

			$status_array = 'low';
			$status       = array( 'text' => __( 'You should rename the wp-content directory of your site.', 'ithemes-security' ), 'link' => '#itsec_enable_content_dir', 'advanced' => true, );
	...
	public function initialize_admin() {
		...
		if ( ! $this->settings === true && isset( $_POST['itsec_enable_content_dir'] ) && $_POST['itsec_enable_content_dir'] == 'true' ) {
			...
			if ( ! wp_verify_nonce( $_POST['wp_nonce'], 'ITSEC_admin_save' ) ) {
				...
				die( __( 'Security check', 'ithemes-security' ) );
	...
	public function metabox_advanced_settings() {
		...
		if ( $this->settings !== true ) {
			...
			$content .= '<p><strong>' . __( 'Please note: Changing the name of your wp-content directory on a site that already has images and other content referencing it will break your site. For this reason, we highly recommend you only try this technique on a fresh WordPress install.', 'ithemes-security' ) . '</strong></p>';
			$content .= '<p>' . __( 'This tool will not allow further changes to your wp-content folder once it has been renamed in order to avoid accidentally breaking the site later. Uninstalling this plugin will not revert the changes made by this page.', 'ithemes-security' ) . '</p>';
			$content .= '<p>' . __( 'Changing the name of the wp-content directory may in fact break plugins and themes that have "hard-coded" it into their design rather than calling it dynamically.', 'ithemes-security' ) . '</p>';
			$content .= sprintf( '<div class="itsec-warning-message"><span>%s: </span><a href="?page=toplevel_page_itsec_backups">%s</a> %s</div>', __( 'WARNING', 'ithemes-security' ), __( 'Backup your database', 'ithemes-security' ), __( 'before using this tool.', 'ithemes-security' ) );
		...
		if ( isset( $itsec_globals['settings']['write_files'] ) && $itsec_globals['settings']['write_files'] === true ) {
					...
					<table class="form-table">
						<tr valign="top">
							<th scope="row" class="settinglabel">
								<label
									for="itsec_enable_content_dir"><?php _e( 'Enable Change Directory Name', 'ithemes-security' ); ?></label>
							</th>
							<td class="settingfield">
								...
								<p class="description"><?php _e( 'Check this box to enable content directory renaming.', 'ithemes-security' ); ?></p>
							</td>
						</tr>
						<tr valign="top" id="content_directory_name_field">
							<th scope="row" class="settinglabel">
								<label
									for="itsec_content_name"><?php _e( 'Directory Name', 'ithemes-security' ); ?></label>
							</th>
							<td class="settingfield">
								<?php //username field ?>
			...
			$content = sprintf(
				...
				__( 'You must allow this plugin to write to the wp-config.php file on the', 'ithemes-security' ),
				__( 'Settings', 'ithemes-security' ),
				__( 'page to use this feature.', 'ithemes-security' )
			);
		...
		if ( strlen( $dir_name ) <= 2 ) { //make sure the directory name is at least 2 characters
			...
			if ( is_array( $configs ) ) {
				...
				if ( ! $renamed ) {
					...
					$type    = 'error';
					$message = __( 'Unable to rename the wp-content folder. Operation cancelled.', 'ithemes-security' );
