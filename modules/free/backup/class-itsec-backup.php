...
class ITSEC_Backup {
		...
		if ( $itsec_files->get_file_lock( 'backup' ) ) {
			...
			if ( $one_time === true ) {
				...
				switch ( $this->settings['method'] ) {
					...
					case 0:
						$details = __( 'emailed to backup recipients and saved locally.', 'ithemes-security' );
						break;
					case 1:
						$details = __( 'emailed to backup recipients.', 'ithemes-security' );
						break;
					default:
						$details = __( 'saved locally.', 'ithemes-security' );
						break;

				}

				$type    = 'updated';
				$message = __( 'Backup Completed and ' . $details, 'ithemes-security' );
			...
			if ( $one_time === true ) {
				...
				$type    = 'error';
				$message = __( 'Something went wrong with your backup. It looks like another process might already be trying to backup your database. Please try again in a few minutes. If the problem persists please contact support.', 'ithemes-security' );
	...
	private function execute_backup( $one_time = false ) {
		...
		if ( $this->settings['method'] !== 2 || $one_time === true ) {
			...
			$body       = __( 'Attached is the backup file for the database powering', 'ithemes-security' ) . ' ' . get_option( 'siteurl' ) . __( ' taken', 'ithemes-security' ) . ' ' . date( 'l, F jS, Y \a\\t g:i a', $itsec_globals['current_time'] );

			//Setup the remainder of the email
			$recipients = $option['backup_email'];
			$subject    = __( 'Site Database Backup', 'ithemes-security' ) . ' ' . date( 'l, F jS, Y \a\\t g:i a', $itsec_globals['current_time'] );
		...
		switch ( $this->settings['method'] ) {
			...
			case 0:
				$status = array(
					'status'  => __( 'Success', 'ithemes-security' ),
					'details' => __( 'emailed to backup recipients and saved locally', 'ithemes-security' ),
				);
