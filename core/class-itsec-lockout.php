...
final class ITSEC_Lockout {
	...
	function add_admin_meta_boxes() {
		...
		add_meta_box(
			'itsec_lockouts',
			__( 'Active Lockouts', 'ithemes-security' ),
	...
	public function get_lockout_description() {
		...
		$description = sprintf(
			...
			__( 'About Lockouts', 'ithemes-security' ),
			__( 'Your lockout settings can be configured in', 'ithemes-security' ),
			__( 'Global Settings', 'ithemes-security' ),
			__( 'Your current settings are configured as follows:', 'ithemes-security' ),
			__( 'Permanently ban', 'ithemes-security' ),
			( $settings['blacklist'] === true ? __( 'yes', 'ithemes-security' ) : __( 'no', 'ithemes-security' ) ),
			__( 'Number of lockouts before permanent ban', 'ithemes-security' ),
			$settings['blacklist_count'],
			__( 'How long lockouts will be remembered for ban', 'ithemes-security' ),
			...
			__( 'Is this computer white-listed', 'ithemes-security' ),
			( $this->is_ip_whitelisted( ITSEC_Lib::get_ip() === true ) ? __( 'yes', 'ithemes-security' ) : __( 'no', 'ithemes-security' ) )
		);
		...
		if ( $itsec_files->get_file_lock( 'lockout_' . $host . $user ) ) {
			...
			if ( $good_host !== false || $good_user !== false ) {
				...
				if ( $good_host !== false && $blacklist_host === false ) { //temp lockout host
					...
					$itsec_logger->log_event( __( 'lockout', 'ithemes-security' ), 10, array( 'expires' => $expiration, 'expires_gmt' => $expiration_gmt, 'type' => $type ), sanitize_text_field( $host ) );
				...
				if ( $good_user !== false ) { //blacklist host and temp lockout user
					...
					if ( $whitelisted === false ) {
						...
						$itsec_logger->log_event( 'lockout', 10, array( __( 'White Listed', 'ithemes-security' ), 'type' => $type ), '', '', intval( $user ) );
					}
	...
	public function lockout_metabox() {
		...
		<form method="post" action="" id="itsec_release_lockout_form">
			...
			<table class="form-table">
				<tr valign="top">
					<th scope="row" class="settinglabel">
						<?php _e( 'Locked out hosts', 'ithemes-security' ); ?>
					</th>
						...
						<?php } else { //no host is locked out ?>
							<ul>
								<li style="list-style: none;">
									<p><?php _e( 'Currently no hosts are locked out of this website.', 'ithemes-security' ); ?></p>
								</li>
							</ul>
						<?php } ?>
					</td>
				...
				<tr valign="top">
					<th scope="row" class="settinglabel">
						<?php _e( 'Locked out users', 'ithemes-security' ); ?>
					</th>
					<td class="settingfield">
						...
						<?php } else { //no user is locked out ?>
							<ul>
								<li style="list-style: none;">
									<p><?php _e( 'Currently no users are locked out of this website.', 'ithemes-security' ); ?></p>
								</li>
							</ul>
						<?php } ?>
					</td>
			...
			<p class="submit"><input type="submit" class="button-primary"
			                         value="<?php _e( 'Release Lockout', 'ithemes-security' ); ?>"/></p>
		</form>
	<?php
	}
	...
	public function register_logger( $logger_modules ) {
		...
		$logger_modules['lockout'] = array(
			'type'     => 'lockout',
			'function' => __( 'Host or User Lockout', 'ithemes-security' ),
		);
	...
	function release_lockout() {
		...
		if ( isset( $_POST['itsec_release_lockout'] ) && $_POST['itsec_release_lockout'] == 'true' ) {
			...
			$type    = 'updated';
			$message = __( 'The selected lockouts have been cleared.', 'ithemes-security' );
	...
	private function send_lockout_email( $host, $user, $host_expiration, $user_expiration, $reason ) {
		...
		$body = sprintf(
			...
			__( 'To change your email preferences please visit', 'ithemes-security' ),
