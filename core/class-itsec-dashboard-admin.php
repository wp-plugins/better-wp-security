...
class ITSEC_Dashboard_Admin {
	...
	public function metabox_normal_status() {
		...
		echo '<li><a href="#itsec_showall">' . __( 'All', 'ithemes-security' ) . '</a></li>';
		echo '<li><a href="#itsec_high">' . __( 'High', 'ithemes-security' ) . '</a></li>';
		echo '<li><a href="#itsec_medium">' . __( 'Medium', 'ithemes-security' ) . '</a></li>';
		echo '<li><a href="#itsec_low">' . __( 'Low', 'ithemes-security' ) . '</a></li>';
		echo '<li><a href="#itsec_completed">' . __( 'Completed', 'ithemes-security' ) . '</a></li>';
		echo '</ul>';
		...
		if ( isset ( $statuses['high'][0] ) ) {
			...
			if ( isset ( $statuses['high'] ) ) {
				...
				$this->status_loop( $statuses['high'], __( 'Fix it', 'ithemes-security' ), 'primary' );
			...
			printf( '<h2>%s</h2>', __( 'High Priority', 'ithemes-security' ) );
			printf( '<p>%s</p>', __( 'You have secured all High Priority items.', 'ithemes-security' ) );
			echo '</div>';
		}
		...
		if ( isset ( $statuses['medium'][0] ) ) {
			...
			_e( 'These are medium priority items that should be fixed if no conflicts are present, but they are not critical to the overall security of your site.', 'ithemes-security' );

			echo '<ul class="statuslist medium-priority">';

			if ( isset ( $statuses['medium'] ) ) {

				$this->status_loop( $statuses['medium'], __( 'Fix it', 'ithemes-security' ), 'primary' );
			...
			echo '<div class="itsec-priority-items-completed">';
			printf( '<h2>%s</h2>', __( 'Medium Priority', 'ithemes-security' ) );
			printf( '<p>%s</p>', __( 'You have secured all Medium Priority items.', 'ithemes-security' ) );
			echo '</div>';
		}
		...
		if ( isset ( $statuses['low'][0] ) ) {
			...
			_e( 'These are low priority items that should be secured if, and only if, your plugins or theme do not conflict with their use.', 'ithemes-security' );

			echo '<ul class="statuslist low-priority">';

			if ( isset ( $statuses['low'] ) ) {

				$this->status_loop( $statuses['low'], __( 'Fix it', 'ithemes-security' ), 'primary' );
			...
			echo '<div class="itsec-priority-items-completed">';
			printf( '<h2>%s</h2>', __( 'Low Priority', 'ithemes-security' ) );
			printf( '<p>%s</p>', __( 'You have secured all Low Priority items.', 'ithemes-security' ) );
			echo '</div>';
		}
		...
		if ( isset ( $statuses['safe-high'] ) || isset ( $statuses['safe-medium'] ) || isset ( $statuses['safe-low'] ) ) {
			...
			_e( 'These are items that you have successfully secured.', 'ithemes-security' );

			echo '<ul class="statuslist completed">';

			if ( isset ( $statuses['safe-high'] ) ) {

				$this->status_loop( $statuses['safe-high'], __( 'Edit', 'ithemes-security' ), 'secondary' );

			}

			if ( isset ( $statuses['safe-medium'] ) ) {

				$this->status_loop( $statuses['safe-medium'], __( 'Edit', 'ithemes-security' ), 'secondary' );

			}

			if ( isset ( $statuses['safe-low'] ) ) {

				$this->status_loop( $statuses['safe-low'], __( 'Edit', 'ithemes-security' ), 'secondary' );
