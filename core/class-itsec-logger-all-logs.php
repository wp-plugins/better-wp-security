...
final class ITSEC_Logger_All_Logs extends ITSEC_WP_List_Table {
	...
	function column_data( $item ) {
		...
		if ( strlen( $data ) > 1 ) {
			...
			$content .= '<a href="itsec-log-all-row-' . $item['id'] . '" class="dialog">' . __( 'Details', 'ithemes-security' ) . '</a>';
	...
	public function get_columns() {
		...
		return array(
			'function' => __( 'Function', 'ithemes-security' ),
			'priority' => __( 'Priority', 'ithemes-security' ),
			'time'     => __( 'Time', 'ithemes-security' ),
			'host'     => __( 'Host', 'ithemes-security' ),
			'user'     => __( 'User', 'ithemes-security' ),
			'url'      => __( 'URL', 'ithemes-security' ),
			'referrer' => __( 'Referrer', 'ithemes-security' ),
			'data'     => __( 'Data', 'ithemes-security' ),
		);
