	...
	final class ITSEC_Core {
		...
		public function admin_bar_links() {
			...
			$wp_admin_bar->add_menu(
			             array(
				             'title' => __( 'Security', 'ithemes-security' ),
			...
			$wp_admin_bar->add_menu(
			             array(
				             ...
				             'title'  => __( 'Dashboard', 'ithemes-security' ),
				             'href'   => admin_url( $network . 'admin.php?page=itsec' ),
		...
		private function admin_modal() {
			...
			echo '<a href="javascript:void(0);" class="itsec-intro-close">' . __( 'Dismiss', 'ithemes-security' ) . '</a>';
			...
			if ( isset( get_current_screen()->id ) && strpos( get_current_screen()->id, 'itsec' ) !== false ) {
				...
				if ( ! isset( $itsec_globals['data']['tooltips_dismissed'] ) || $itsec_globals['data']['tooltips_dismissed'] === false ) {
					...
					wp_localize_script( 'itsec_modal', 'itsec_tooltip_text', array(
						...
						'messages' => $messages,
						'title' => __( 'Important First Steps', 'ithemes-security' ),
					) );
		...
		public function register_tooltip( $tooltip_modules ) {
			...
			$tooltip_modules['one-click'] = array(
				...
				'heading'   => __( 'Secure Your Site', 'ithemes-security' ),
				'text'      => __( 'Use the button below to enable default settings. This feature will enable all settings that cannot conflict with other plugins or themes.', 'ithemes-security' ),
				'link_text' => __( 'One-Click Secure', 'ithemes-security' ),
				'callback'  => array( $this, 'tooltip_ajax' ),
				'success'   => __( 'Site Secured. Check the dashboard for further suggestions on securing your site.', 'ithemes-security' ),
				'failure'   => __( 'Whoops. Something went wrong. Please contact support if the problem persists.', 'ithemes-security' ),
			);
