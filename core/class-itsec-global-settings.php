...
class ITSEC_Global_Settings {
	...
	public function add_admin_meta_boxes() {
		...
		add_meta_box(
			...
			__( 'Quick Links', 'ithemes-security' ),
	...
	public function add_module_advanced_intro() {
		...
		printf(
			'<p>%s %s%s%s %s</p>',
			__( 'The settings below are more advanced settings that should be done with caution on an existing site.', 'ithemes-security' ),
			$backup_link_open,
			__( 'Make sure you have a good backup before changing any setting on this page.', 'ithemes-security' ),
			$backup_link_close,
			__( 'In addition, these settings will not be reversed if you remove this plugin. That said, all settings on this page use methods recommended by WordPress.org itself and will help in improving the security of your site.', 'ithemes-security' )
		);
	...
	public function allow_tracking() {
		...
		echo '<label for="itsec_global_allow_tracking">' . __( 'Allow iThemes to track plugin usage via anonymous data. ', 'ithemes-security' ) . '</label>';
	...
	public function backup_email() {
		...
		echo '<p class="description">' . __( 'The email address(es) all database backups will be sent to. One address per line.', 'ithemes-security' ) . '</p>';
	...
	public function blacklist() {
		...
		echo '<label for="itsec_global_blacklist"> ' . __( 'Enable Blacklist Repeat Offender', 'ithemes-security' ) . '</label>';
		echo '<p class="description"> ' . __( 'If this box is checked the IP address of the offending computer will be added to the "Ban Users" blacklist after reaching the number of lockouts listed below.', 'ithemes-security' ) . '</p>';

		if ( ITSEC_Lib::get_server() == 'nginx' ) {

			echo '<p class="description"> ' . __( 'Note that as you are on NGINX you will still need to manually restart the server even though the users will be added to the banned users list.', 'ithemes-security' ) . '</p>';
	...
	public function blacklist_count() {
		...
		echo '<label for="itsec_global_blacklist_count"> ' . __( 'Lockouts', 'ithemes-security' ) . '</label>';
		echo '<p class="description"> ' . __( 'The number of lockouts per IP before the host is banned permanently from this site.', 'ithemes-security' ) . '</p>';
	...
	public function blacklist_period() {
		...
		echo '<label for="itsec_global_blacklist_period"> ' . __( 'Days', 'ithemes-security' ) . '</label>';
		echo '<p class="description"> ' . __( 'How many days should a lockout be remembered to meet the blacklist count above.', 'ithemes-security' ) . '</p>';
	...
	public function email_notifications() {
		...
		echo '<label for="itsec_global_email_notifications">' . __( 'Enable Email Lockout Notifications', 'ithemes-security' ) . '</label>';
		printf( '<p class="description">%s<a href="admin.php?page=toplevel_page_itsec_settings">%s</a>%s</p>', __( 'This feature will trigger an email to be sent to the ', 'ithemes-security' ), __( 'notifications email address', 'ithemes-security' ), __( ' whenever a host or user is locked out of the system.', 'ithemes-security' ) );
	...
	public function infinitewp_compatibility() {
		...
		echo '<label for="itsec_global_email_notifications">' . __( 'Enable InfiniteWP Compatibility', 'ithemes-security' ) . '</label>';
		printf(
			'<p class="description">%s <a href="http://infinitewp.com" target=""_blank">%s</a> %s</p>',
			__( 'Turning this feature on will enable compatibility with', 'ithemes-security' ),
			__( 'InfiniteWP.', 'ithemes-security' ),
			__( 'Do not turn it on unless you use the InfiniteWP service.', 'ithemes-security' )
		);
	...
	public function initialize_admin() {
		...
		add_settings_field(
			...
			__( 'Write to Files', 'ithemes-security' ),
		...
		add_settings_field(
			...
			__( 'Backup Delivery Email', 'ithemes-security' ),
		...
		if ( is_dir( WP_PLUGIN_DIR . '/iwp-client' ) ) {
			...
			add_settings_field(
				...
				__( 'Add InfiniteWP Compatibility', 'ithemes-security' ),
		...
		if ( ITSEC_Lib::get_server() == 'nginx' ) {
			...
			add_settings_field(
				...
				__( 'NGINX Conf File', 'ithemes-security' ),
	...
	public function lockout_message() {
		...
		echo '<p class="description">' . __( 'The message to display when a computer (host) has been locked out.', 'ithemes-security' ) . '</p>';
	...
	public function lockout_period() {
		...
		echo '<label for="itsec_global_lockout_period"> ' . __( 'Minutes', 'ithemes-security' ) . '</label>';
		echo '<p class="description"> ' . __( 'The length of time a host or user will be banned from this site after hitting the limit of bad logins.', 'ithemes-security' ) . '</p>';
	...
	public function lockout_white_list() {
		...
		echo '<ul>';
		echo '<li>' . __( 'You may white list users by individual IP address or IP address range.', 'ithemes-security' ) . '</li>';
		echo '<li>' . __( 'Individual IP addesses must be in IPV4 standard format (i.e. ###.###.###.### or ###.###.###.###/##). Wildcards (*) or a netmask is allowed to specify a range of ip addresses.', 'ithemes-security' ) . '</li>';
		echo '<li>' . __( 'If using a wildcard (*) you must start with the right-most number in the ip field. For example ###.###.###.* and ###.###.*.* are permitted but ###.###.*.### is not.', 'ithemes-security' ) . '</li>';
		echo '<li><a href="http://ip-lookup.net/domain-lookup.php" target="_blank">' . __( 'Lookup IP Address.', 'ithemes-security' ) . '</a></li>';
		echo '<li>' . __( 'Enter only 1 IP address or 1 IP address range per line.', 'ithemes-security' ) . '</li>';
		echo '</ul>';
	...
	public function log_location() {
		...
		echo '<label for="itsec_global_log_location"> ' . __( 'The path on your server where log files should be stored.', 'ithemes-security' ) . '</label>';
		echo '<p class="description"> ' . __( 'This path must be writable by your website. For added security it is recommended you do not include it in your website root folder.', 'ithemes-security' ) . '</p>';
	...
	public function log_rotation() {
		...
		echo '<label for="itsec_global_log_rotation"> ' . __( 'Days', 'ithemes-security' ) . '</label>';
		echo '<p class="description"> ' . __( 'The number of days database logs should be kept. File logs will be kept indefinitely but will be rotated once the file hits 10MB.', 'ithemes-security' ) . '</p>';
	...
	public function nginx_file() {
		...
		echo '<label for="itsec_backup_nginx_file"> ' . __( 'The path on your server where backup files should be stored.', 'ithemes-security' ) . '</label>';
		echo '<p class="description"> ' . __( 'This path must be writable by your website. For added security, it is recommended you do not include it in your website root folder.', 'ithemes-security' ) . '</p>';
	...
	public function notification_email() {
		...
		echo '<p class="description">' . __( 'The email address(es) all security notifications will be sent to. One address per line.', 'ithemes-security' ) . '</p>';
	...
	public function register_tooltip( $tooltip_modules ) {
		...
		if ( get_site_transient( 'ITSEC_SHOW_WRITE_FILES_TOOLTIP' ) || ( ! isset ( $this->settings['write_files'] ) || $this->settings['write_files'] === false ) ) {
			...
			$tooltip_modules['writing'] = array(
				...
				'heading'   => __( 'Allow File Updates', 'ithemes-security' ),
				'text'      => __( 'Many of the functions of this plugin require editing your wp-config.php or .htaccess files. Would you like to allow us to safely update these files for you automatically?', 'ithemes-security' ),
				'link_text' => __( 'Allow file updates', 'ithemes-security' ),
				'callback'  => array( $this, 'tooltip_ajax_writing' ),
				'success'   => __( 'Setting Saved. File updates allowed.', 'ithemes-security' ),
				'failure'   => __( 'Whoops. Something went wrong. Check the "Global Settings" section on the settings page (it is the first setting) to make sure your option was saved or contact support.', 'ithemes-security' ),
			);
		...
		if ( ! isset ( $this->settings['allow_tracking'] ) || $this->settings['allow_tracking'] === false ) {
			...
			$tooltip_modules['tracking'] = array(
				...
				'heading'   => __( 'Help Us Improve', 'ithemes-security' ),
				'text'      => sprintf( '%s %s', $itsec_globals['plugin_name'], __( 'would like to collect anonymous data about features you use to help improve this plugin. Absolutely no information that can identify you will be collected.', 'ithemes-security' ) ),
				'link_text' => __( 'Yes, I\'d like to help', 'ithemes-security' ),
				'callback'  => array( $this, 'tooltip_ajax_tracking' ),
				'success'   => __( 'Setting Saved. Thanks for helping us make this plugin better.', 'ithemes-security' ),
				'failure'   => __( 'Whoops. Something went wrong. Check the global settings page or contact support.', 'ithemes-security' ),
			);
	...
	public function user_lockout_message() {
		...
		echo '<p class="description">' . __( 'The message to display to a user when their account has been locked out.', 'ithemes-security' ) . '</p>';
	...
	public function write_files() {
		...
		printf(
			...
			__( 'Allow', 'ithemes-security' ),
			$itsec_globals['plugin_name'],
			__( 'to write to wp-config.php', 'ithemes-security' ),
			$server_file
		);
		printf(
			'<p class="description">%s %s %s%s%s</p>',
			__( 'Whether or not', 'ithemes-security' ),
			$itsec_globals['plugin_name'],
			__( 'should be allowed to write to wp-config.php', 'ithemes-security' ),
			$server_file,
			__( ' automatically. If disabled you will need to manually place configuration options in those files.', 'ithemes-security' )
		);
