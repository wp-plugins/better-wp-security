...
final class ITSEC_Files {
	...
	function add_admin_meta_boxes() {
		...
		add_meta_box(
			'itsec_rewrite',
			__( 'Rewrite Rules', 'ithemes-security' ),
		...
		add_meta_box(
			'itsec_wpconfig',
			__( 'wp-config.php Rules', 'ithemes-security' ),
		...
		if ( $this->rewrites_changed === true ) {
			...
			if ( is_array( $rewrites ) ) {
				...
				if ( $rewrites['success'] === false ) {
					...
					add_settings_error( 'itsec', esc_attr( 'settings_updated' ), __( 'Settings Updated', 'ithemes-security' ) . '<br />' . $message, $type );
	...
	public function config_metabox_contents() {
		...
		if ( strlen( $rules_to_write ) > 1 ) {
			...
			_e( 'There are no rules to write.', 'ithemes-security' );
	...
	public function save_rewrites() {
		...
		if ( ITSEC_Lib::get_server() == 'nginx' || ( isset( $itsec_globals['settings']['write_files'] ) && $itsec_globals['settings']['write_files'] === true ) ) {
			...
			if ( $this->get_file_lock( 'htaccess' ) ) {
				...
				if ( $success === true && $release === true ) {
					...
					if ( ITSEC_Lib::get_server() == 'nginx' ) {
						...
						return array(
							...
							'text'    => sprintf(
								'%s %s. %s',
								__( 'Your rewrite rules have been saved to', 'ithemes-security' ),
								$itsec_globals['settings']['nginx_file'],
								__( 'You must restart your NGINX server for the settings to take effect', 'ithemes-security' )
							),
						);
					...
					return array(
						'success' => false,
						'text'    => __( 'Unable to release a lock on your .htaccess or nginx.conf file. If the problem persists contact support.', 'ithemes-security' ),
					);
					...
					return array(
						'success' => false,
						'text'    => __( 'Unable to write to your .htaccess or nginx.conf file. If the problem persists contact support.', 'ithemes-security' ),
					);
				...
				return array(
					'success' => false,
					'text'    => __( 'WordPress was unable to save the your options to .htaccess or nginx.conf file. It looks like another process might already be working on the file. Please wait a few minutes and try again or contact support if the problem persists.', 'ithemes-security' ),
				);
		...
		if ( isset( $itsec_globals['settings']['write_files'] ) && $itsec_globals['settings']['write_files'] === true ) {
			...
			if ( $this->get_file_lock( 'wpconfig' ) ) {
				...
				if ( $success === true && $release === true ) {
					...
					return array(
						'success' => false,
						'text'    => __( 'Unable to release a lock on your wp-config.php file. If the problem persists contact support.', 'ithemes-security' ),
					);;
					...
					return array(
						'success' => false,
						'text'    => __( 'Unable to write to your wp-config.php file. If the problem persists contact support.', 'ithemes-security' ),
					);
				...
				return array(
					'success' => false,
					'text'    => __( 'WordPress was unable to save the your options to wp-config.php. It looks like another process might already be working on the file. Please wait a few minutes and try again or contact support if the problem persists.', 'ithemes-security' ),
				);
